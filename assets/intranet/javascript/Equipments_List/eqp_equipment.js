...
function change_page_video(num_page)
{
	$('page').value = num_page;
	indexKeyBoardCursor = -1;
	search_equipments_video();
...
function search_equipments()
{
	...
	*/
	...
	var str_netlogin_status = '';
	if( $('is_netlogin_0').checked )
	{
		str_netlogin_status = '&netlogin_status=0';
	}	
	if( $('is_netlogin_1').checked )
	{
		str_netlogin_status = '&netlogin_status=1';
	}	
	...
	var str_request = '?todo=list' + 
					  ...
					  '' + str_deployment + 
					  '' + str_monitoring + 
					  '&ip_address=' + encodeURIComponent(($('ip_address_search').value).trim()) +  
...
function subnet_search_check_ip_address()
{
	var ret_fct = true;
	var val_subnet_mask = $('subnet_mask_search').value;
	var val_ip_addr 	= ($('ip_address_search').value).trim();
...
function call_search_equipment_video()
{
	$('page').value = 0;
	indexKeyBoardCursor = -1;
	search_equipments_video();
...
function search_equipments_video()
{
	/*
	if( $('content_list_equipments').style.display == 'none' )
	{
	...
	*/
	...
	var str_request = '?todo=list' + 
					  ...
					  '&sort=' + $('sort').value; 
	//prompt('', str_request);
	call_ajax_page( 'get_list_equipments_video.php' + str_request , 'content_list_equipments', call_ajax_post_equipments );
}