 ...
 */
	...
	/**
 	 * Recherche d'utilisateur avec email (un seul email)
 	 */
	function search_spec_email(event, inputField, completionDiv)
	{
		common_search_user(event, inputField, null, completionDiv, completion_email, complete_email, 'single_email');
	}
 ...
 */
	...
	function common_search_user(event, inputField, outputField, completionDiv, noentry_callback_function, ajaxdone_callback_function, type, option)
	{
		...
		// Url Ajax.
		var url = '/base_loisirs.php/ajax/user_data/?user=';
	
		// Si utilisateurs IT : url particulière.
		if( type == 'access' )
		{
			url = 'http://intranet.perenco.net/js/template_v2/ajax/php/access.php?keyword=';
		}
...
function common_completion_user( event, saveField, inputField, outputField, completionDiv, type )
{
		...
		&& completion.selectedIndex >= 0 )
	{
		...
		var callback = input.getAttribute("callback");
		var callback_attr = input.getAttribute("callback_attr");
		var fn_callback = window[callback];
		if(typeof fn_callback === 'function') {
			fn_callback(callback_attr);
		}
	}
	else
	{
