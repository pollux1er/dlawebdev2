	...
	<table class="planning" id="planning">
		...
		<tbody>
			...
			<?php 		} 
				...
				?><!--
				<td name="day" id="9001:1" class="day" style="background-color: rgb(255, 128, 0);"></td>
				<td name="day" id="9001:2" class="WE" style="background-color: rgb(255, 128, 0);"></td>
			 rgb(128, 128, 128);"></td>
				<td name="day" id="9001:39" class="day l finger" style="background-color: rgb(128, 128, 128);"></td>
				<td name="day" id="9001:40" class="day l finger" style="background-color: rgb(128, 128, 128);"></td>
				<td name="day" id="9001:41" class="day l finger" style="background-color: rgb(128, 128, 128);"></td>
				<td name="day" id="9001:42" class="day l finger" style="background-color: rgb(128, 128, 128);"></td>
			...
			<tr class="header">
				...
				<?php 
					...
					$period = new DatePeriod($begin, $interval, $stop);
					 ...
					 ?>
						<td class="dayW" colspan="<?php echo $nbjrs; ?>"></td>
		
				
				
			</tr>
			<?php } ?>
			<tr id="" class="user_line">
				<td class="user" ref="13072">Attente 1er rang</td>
				<?php 
					$begin = new DateTime( $start_date );
					$stop = new DateTime( $end_date );
					$stop = $stop->modify( '+1 day' );
					...
					$occupancy = $this->demande->check_case_jour_pending($d->format( 'Y-m-d' ));
					if($occupancy) {
						$lettre = substr($occupancy, -1);
							//$occupancy = substr($occupancy, 0, 1);
						$case  = $occupancy[0];
					...
					if($occupancy) {
						...
						$class = 'class="day l '.$t.'" style="background-color : '.$color.'; font-size: 12px; line-height: 19px; cursor: pointer;"';
						$occupancygc = substr($d->format( 'd F Y (D)' ) . $occupancy, 0, -1);
						?><td <?php echo $class; ?> onMouseOver="popupOutline('<?php echo $occupancygc; ?>');" onMouseOut="return nd();" style="background-color: white !important;color: white;"><?php echo $lettre; ?></td><?php
					} else {
					?><td <?php echo $class; ?>></td>
		...
		<?php 		} 
			...
			<tr id="" class="user_line">
				<td class="user" ref="13072">Attente 2eme rang</td>
				<?php 
					$begin = new DateTime( $start_date );
					$stop = new DateTime( $end_date );
					$stop = $stop->modify( '+1 day' );
					...
					$occupancy = $this->demande->check_case_jour_pending($d->format( 'Y-m-d' ), 2);
					if($occupancy) {
						$lettre = substr($occupancy, -1);
							//$occupancy = substr($occupancy, 0, 1);
						$case  = $occupancy[0];
