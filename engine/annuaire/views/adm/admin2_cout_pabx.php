...
<div style="border:1px solid gray;" class="tpl_box_init_container">
	...
	<div class="alert alert-info" style="background-color: #51647d;margin: 1% 1% 0;text-align: left;width: 98%;">
					...
					<form style="margin : 0 1%;" action="" target="_self">
						<div class="form-group">
							<div class="input-group">
								...
								<a href="<?php echo base_url('annuaire.php/annuaire/load_csv'); ?>" title="Charger le fichier des consomations" target="_self">
									<img style="margin: 0;float: right; height: 46px; margin-right: 1%;; padding: 0%;" src="assets/annuaire/images/excel_2013.png" alt="submit" />
								</a>
								<a href="<?php echo base_url('annuaire.php/annuaire/add_new_pabx'); ?>" title="Ajouter une nouvelle entrée PABX" target="_self">
					...
					<table cellspacing="0" cellpadding="0" class="table-hover tab_op_list_main table table-bordered table-striped" style="width : 98%" >
						...
						<tbody>
							 <tr id="{{ item.Id }}" align="left" ng-repeat="item in Annuaire | orderBy:sortType:sortReverse  | filter:Noms | filter:Mouchard | filter:Habilitation | filter:Fonction | filter:Departement | filter:Extension ">
						        ...
						        <td class="extension" style="text-align: center">
									<span editable-text="item.Abrege" e-name="Abrege" e-form="rowform" onbeforesave="checkName($data, item.Id)">
										{{ item.Abrege || '' }}
									</span>
								</td>
						        <td>
  ...
  $scope.Annuaire = [
	...
	foreach ($users as $row) { 	
		...
		{ Id: <?php echo "'" . $row->id . "'"; ?>, 
			...
			Abrege: <?php echo "'" . utf8_decode($row->abrege) . "'"; ?> ,
...
?>
  ...
  $scope.removeUser = function(index, id, ext) {
  	if (confirm("Etes vous sur de vouloir effacer cette entrée ?")) {
    	...
    	//angular.extend({}, {id_user: id, ext: ext});
	   	return $http.post('<?php echo base_url(); ?>annuaire.php/ajax/delete_taxation_infos', {id_user: id, ext: ext})
  ...
  $scope.saveUser = function(data, id) {
	...
	//alert(data);
	   ...
	   return $http.post('<?php echo base_url(); ?>annuaire.php/ajax/update_taxation_infos', data);
  };
