						...
						<tbody>
							...
							<tr align="center" style="" id="tr<?php echo $row->id_demandes; ?>" class="<?php if( $i & 1 ) echo "even"; else echo "odd"; ?> line_color_0 tr_survol ">
								...
								<td align="left" style="padding:0 2px 0 2px">
									...
									<a href="http://localhost/dlawebdev2/base_loisirs.php/ajax/receipt_resa/?id_resa=<?php echo 'a' . $row->id_demandes; ?>" target="_blank" id="a<?php echo $row->id_demandes; ?>" class="receipt"><img class="tips" id="<?php echo $row->id_demandes; ?>" title="Imprimer Recu" style="margin:2px 0 2px 10px; cursor:help;  float:left" src="assets/base_loisirs/images/receipt_invoice.png"></a>
								</td>
							</tr>
							<?php $i++; } ?>
...
{	
	...
	var Recu = new Request({
		url: 'base_loisirs.php/ajax/receipt_resa/',
		method: 'get',
		onProgress: function(event, xhr){
			var loaded = event.loaded, total = event.total;
		...
		onComplete: function(response){
			...
			// } else {
				// <?php echo base_url(); ?>
			// } 
			//window.location.href = '/base_loisirs.php/ajax/receipt_resa/';
		}
	});
	...
	// $$('.receipt').addEvent('click', function(e){
		// e.stop();
		// var id = this.id;
		// var SM = new SimpleModal({"btn_ok":"Confirmer"});
        // SM.show({
          // "model":"confirm",
          // "callback": function(){
			// http://localhost/dlawebdev2/base_loisirs.php/ajax/receipt_resa/?id_resa=
          // },
          // "title":"Imprimer le reçu ?",
          // "contents":"Vous etes sur le point de lancer la génération du recu de cette reservation. Procéder ?"
