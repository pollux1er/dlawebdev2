...
class Notify_model extends CI_Model {
	...
	public function reservation_effectuee($id_resa)
	{
		...
		if($row['days'] > 6) {
			...
			$d = "Vous avez " . ($row['days'] - 5) . " jours pour confirmer votre r&eacute;servation en r&eacute;glant vos frais (".$row['frais']." Fcfa) aupr&egrave;s de Marie AVA ATANGA (porte 317).<br /> Apr&egrave;s ce d&eacute;lai, votre r&eacute;servation sera automatiquement annul&eacute;e.";
		}
		else {
			//$delais = "<br /><span style='color:red:'>Vous avez cette journ&eacute;e pour confirmer votre r&eacute;servation!</span><br />";
			$d = "Vous avez cette journ&eacute;e pour confirmer votre r&eacute;servation en r&eacute;glant vos frais (".$row['frais']." Fcfa) aupr&egrave;s de Marie AVA ATANGA (porte 317).<br /> Apr&egrave;s ce d&eacute;lai, votre r&eacute;servation sera automatiquement annul&eacute;e.";
		}
	...
	public function modification_effectuee($id_resa, $data = null)
	{
		// On selectionne la derniere reservation effectuée par l'utilisateur
		$query = $this->db->query("SET lc_time_names = 'fr_FR'");
		$query = $this->db->query(" SELECT u.First_Name, u.`Professional E-mail`, rv.id_case, frais, `date_arrivee` as arrivee, DATE_FORMAT(`date_arrivee`, '%W %d %M %Y') as date_arr, statut, DATE_FORMAT(`date_arrivee`, '%H:%i') as heure_arr, `date_depart` as depart, DATE_FORMAT(`date_depart`, '%W %d %M %Y') as date_dep, DATE_FORMAT(`date_depart`, '%H:%i') as heure_dep, datediff(DATE_FORMAT(`date_arrivee`, '%Y-%m-%d'), now()) as days, c.nom as nomcase FROM bl_demandes AS rv 
									LEFT JOIN userscm AS u ON u.user_id = rv.id_user_staff 
									LEFT JOIN bl_case AS c ON c.id_case = rv.id_case 
									WHERE id_user_staff = '".$this->session->userdata('iduser')."' ORDER BY id_demandes DESC LIMIT 1;");
		$row = $query->row_array();
		...
		$delais = "Si oui, les frais de r&eacute;servation (".$row['frais']." Fcfa) sont attendus au bureau 317. <br />";
		...
		$ToName = $ToName = strtr($row['First_Name'], $trans);
		///////////////////////////////////////// Change management appearance in mails ////////////////////////////
		$case = "<td><strong>".$row['nomcase']."</strong></td>";
		if($this->session->userdata('id_case_old') != $row['id_case']) {
			$case = "<td style='background-color:yellowgreen;'><strong>".$row['nomcase']."</strong></td>";
		}
		$arrivee = "<td><strong>".strtr(ucwords($row['date_arr']), $trans). " &agrave " . $row['heure_arr'] ."</strong></td>";
		if($this->session->userdata('arrivee_old') != $row['arrivee']) {
			$arrivee = "<td style='background-color:yellowgreen;'><strong>".strtr(ucwords($row['date_arr']), $trans). " &agrave " . $row['heure_arr'] ."</strong></td>";
		}
		$depart = "<td><strong>".strtr(ucwords($row['date_dep']), $trans). " &agrave " . $row['heure_dep'] ."</strong></td>";
		if($this->session->userdata('depart_old') != $row['depart']) {
			$depart = "<td style='background-color:yellowgreen;'><strong>".strtr(ucwords($row['date_dep']), $trans). " &agrave " . $row['heure_dep'] ."</strong></td>";
		}
		////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$mail->AddAddress($To , $ToName);
		$mail->AddCC('passontia@cm.perenco.com', 'Patient Assontia');
	...
	public function alerte_7jrs($donnees_pers)
	{
		...
		$mail->Body     =  "Bonjour $ToName, <br /><br />
							...
							<br />
							Vous devez confirmer votre r&eacute;sservation en r&eacute;glant vos frais aupr&egrave;s du gestionnaire de la base de Kribi (Porte 317). <br />
	...
	public function alerte_6jrs($donnees_pers)
	{
		...
		$mail->Body     =  "
							...
							*
							...
							Vous devez confirmer votre r&eacute;servation en r&eacute;glant vos frais aupr&egrave;s du gestionnaire de la base de Kribi (Porte 317). <br />
							Apr&egrave;s ce d&eacute;lai, votre r&eacute;servation sera automatiquement mise en attente et la case remise &agrave; disposition.<br />
							Cliquez sur les liens suivants pour <a href='".base_url()."base_loisirs.php/demande/'>annuler ou modifier votre r&eacute;servation</a> ou consulter <a href='".base_url()."base_loisirs.php/attendance/'>le Planning des cases</a>.<br /><br />
							<strong><u>NB</u></strong> : Ouverture des liens possible uniquement &agrave; partir de votre poste de travail.<br /><br />
							<hr size=100% class='bleu' />
	...
	public function alerte_5jrs($donnees_pers)
	{
		...
		$mail->Body     =  "
							...
							*
							...
							Vous pouvez encore confirmer ou annuler votre r&eacute;servation (Porte 317) ou consulter le planning en cliquant sur le lien suivant : <a href='".base_url()."base_loisirs.php/attendance/'>Planning des cases</a>.<br /><br />
							<strong><u>NB</u></strong> : Ouverture des liens possible uniquement &agrave; partir de votre poste de travail.<br /><br />
							<hr size=100% class='bleu' />
							Cet e-mail a &eacute;t&eacute; envoy&eacute; automatiquement, merci de NE PAS REPONDRE.";
	...
	public function alerte_3jrs($donnees_pers)
	{
		...
		$mail->Body     =  "Bonjour $ToName, <br /><br />
							Le d&eacute;lai pour confirmer votre r&eacute;servation expire dans 24h; <br />
							Vous devez confirmer votre r&eacute;sservation en r&eacute;sglant vos frais aupr&egrave;s du gestionnaire de la base de Kribi (porte 317). <br />
							Apr&egrave;s ce d&eacute;lai, votre r&eacute;servation sera automatiquement mise en attente et la case remise &agrave; disposition.<br />
