...
class Demande_model extends CI_Model {
	...
	public function modifier_reservation($data, $statut = null)
	{
		...
		if($statut == '1') {
			...
			// avant envoi de la nouvelle date
			$data['arrivee'] = $arr_date;
			$data['depart'] = $dep_date;
			$this->notify->modification_effectuee($id_resa, $data);		
		}
		return $this->db->affected_rows();	
	}
	...
	public function get_all($get = null)
	{
		...
		$query = $this->db->query(" SELECT bl.id_demandes, bl.motif, bl.frais, bc.nom as nomcase, Last_Name, First_Name, DATE_FORMAT(`date_arrivee`, '%d %M') as date_arrivee, DATE_FORMAT(`date_arrivee`, '%H:%i') as heure_arrivee, DATE_FORMAT(`date_depart`, '%H:%i') as heure_depart, DATE_FORMAT(`date_depart`, '%d %M') as date_depart, `statut`, DATE_FORMAT(`date_created`, '%W %d %M') as date, DATE_FORMAT(`date_created`, '%H:%i:%s') as heure, bl.id_case, statut, datediff(DATE_FORMAT(`date_arrivee`, '%Y-%m-%d'), now()) as days   
	...
	public function afficher($id_resa = null)
	{
		if(!is_null($id_resa)) {
			...
			$query = $this->db->query("SELECT `id_demandes`, `id_user_staff`, `date_arrivee` as arrivee, DATE_FORMAT(`date_arrivee`, '%d/%m/%Y') as date_arrivee, DATE_FORMAT(`date_arrivee`, '%H:%i') as heure_arrivee, `date_depart` as depart, DATE_FORMAT(`date_depart`, '%d/%m/%Y') as date_depart, DATE_FORMAT(`date_depart`, '%H:%i') as heure_depart, `invites`, `nb_invites`, `details`, `statut`, `id_case`, `id_saison`, `frais`, `ip_user` FROM `bl_demandes` WHERE id_demandes = '".$id_resa."' LIMIT 1;");
	...
	public function get_saison_from_date($date)
	{
		...
		// if($this->input->ip_address() == '10.52.66.78') {
			// var_dump("SELECT * FROM `bl_saison` WHERE '$date' BETWEEN date_debut AND date_fin LIMIT 1;");
			// exit;
		// }
		if ($query->num_rows() == 0) 
			return false;
	...
	public function check_case_jour($id_case, $date)
	{
		...
		if ($query->num_rows() == 0) { // Si on n'a aucune reservation alors false
			...
			// Si les 2 sont resas
			if($r == 2) {
				$html = '.' . "<br><b style=\'color:white\'>Entree et Sortie</b>";
				$html .= "<br />";
