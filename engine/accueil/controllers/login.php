...
class Login extends PRDR_Controller{
	...
	public function index($message = ''){
		...
		//--------------------------------------- fin test --------------------------------------//
		//var_dump($this->session->userdata('iduser')); die;
		if ($this->session->userdata('iduser')) {
				redirect(base_url("home.php"));
		}
		
		$this->load->helper(array('form'));
		$this->load->view('perenco\index', array('message' => $message, 'method'=> base_url("accueil.php/login/enter")));
		...
		// $host = "co.sub.intranetqal.perenco.net";
